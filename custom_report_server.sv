...
`ifdef CUSTOM_REPORT_MACROS
        if( // Проверяем сообщения только тогда, когда:
            !no_color &&           // 1) Цвета включены
            message.len() > 100 && // 2) Длина сообщения больше 100 символов (т.к. макросы длинные)
            (message[1] == "#" ||  // 3) Сообщение начинается с #
            message[1] == "!")     // или ! (по формату макросов)
        )
            case(message)
